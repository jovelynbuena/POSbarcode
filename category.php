...
if(isset($_POST['btnsave'])) {
    ...
    elseif(strlen($category) > 50) {
        $_SESSION['status'] = "Category name should not exceed 50 characters";
        $_SESSION['status_code'] = "warning";
    } 
   
    elseif(!preg_match('/^[a-zA-Z0-9\s]+$/', $category)) {
        $_SESSION['status'] = "Category name should not contain special characters";
        $_SESSION['status_code'] = "warning";
    } 
    else {
      
        $checkCategory = $pdo->prepare("SELECT * FROM tbl_category WHERE category = :cat");
        $checkCategory->bindParam(':cat', $category);
